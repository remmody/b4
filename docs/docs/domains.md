...
Допустим, вы видите в таблице `facebook.com` без бейджа Set — значит этот домен ещё не в списке обхода.

1. Кликните на строку с доменом (по иконке плюс рядом с доменом ➕)
2. В модальном окне выберите вариант:
   - `facebook.com` — точное совпадение
   - `*.facebook.com` — все поддомены
   - `*.com` — весь TLD (обычно не нужно)
3. Выберите Set, в который добавить домен
...
Если вы ранее добавляли IP через RIPE-обогащение, B4 запоминает ASN. При следующих соединениях к этому провайдеру в колонке Destination появится жёлтый бейдж с названием организации.

---

## Что дальше

- [Настройка Sets](/docs/sets) — куда попадают добавленные домены
- [API интеграции](/docs/api) — настройка IPInfo токена
